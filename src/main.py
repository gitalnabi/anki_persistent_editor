...
def do_not_require_from_editcurrent(should_reset, reason, context):
    return (
        False
        if (
            reason == ResetReason.EditCurrentInit
            or reason == ResetReason.EditorBridgeCmd
            and isinstance(context.parentWindow, EditCurrent)
        )
        else should_reset
    )
...
def close_editcurrent_when_leaving_review(state, oldstate):
    if (
        oldstate == "review"
        and state in ["overview", "deckBrowser"]
        and (editcurrent := dialogs._dialogs["EditCurrent"][1])
    ):
